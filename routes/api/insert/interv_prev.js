...
//create intervention
router.post('/',(req,res) => {
  const newInterv = {N_maintenance : req.body.N_maintenance ,date_heure_maintenance : req.body.date_heure_maintenance , procedure_maintenance : req.body.procedure_maintenance , remarque_maintenance : req.body.remarque_maintenance ,temps_arret : req.body.temps_arret ,temps_maint : req.body.temps_maint ,libelle_equip : req.body.libelle_equip ,intervenant : req.body.intervenant , stock : req.body.stock};
  if(!newInterv.N_maintenance || !newInterv.date_heure_maintenance || !newInterv.procedure_maintenance || !newInterv.remarque_maintenance || !newInterv.temps_arret || !newInterv.temps_maint || !newInterv.libelle_equip || !newInterv.intervenant  || !newInterv.stock )
    {return res.status(400).json({ msg : 'please include all entry'}) ;}
  var sql_query = "SELECT N_maintenance FROM Intervention_preventive UNION SELECT N_maintenance FROM Intervention_corrective ";//UNION SELECT N_maintenance FROM Intervention_corrective
  connection.query(sql_query , (err,result,fields) => {
      ...
      connection.beginTransaction((err) => {
        ...
        var sql_query = "INSERT INTO Intervention_preventive VALUES ?";
        values = [[newInterv.N_maintenance,newInterv.date_heure_maintenance,newInterv.procedure_maintenance,newInterv.remarque_maintenance,newInterv.temps_arret,newInterv.temps_maint,newInterv.libelle_equip]];
